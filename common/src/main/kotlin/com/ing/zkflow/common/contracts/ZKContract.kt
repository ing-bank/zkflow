...
private fun LedgerTransaction.ensureFilteredLedgerTransaction(): LedgerTransaction {
    val metadata = zkTransactionMetadataOrNull() ?: return this
    return if (inFilteredContext()) this else filterPubliclyVisible(metadata)
}

// Filters out components that should not be publicly visible according to the metadata.
fun LedgerTransaction.filterPubliclyVisible(metadata: ResolvedZKTransactionMetadata): LedgerTransaction {
