...
object SortedStateFamiliesRegistry {
    ...
    private fun KSClassDeclaration.getPredecessor(declarationsOfThisFamily: List<KSClassDeclaration>): KSClassDeclaration? {
        ...
        val declarationNamesSet = declarationsOfThisFamily.map { it.toString() }.toSet()
        val referredParameterType = constructorParameterTypes.filter { typeName ->
            typeName != toString() && typeName in declarationNamesSet
        }.singleOrNull()
        return declarationsOfThisFamily.singleOrNull { it.toString() == referredParameterType }
    }
    ...
    private fun sortStateDeclarations(
        ...
        var currentElement = declarations.single { predecessorMap[it] == null } // first element
    ...
    public fun buildSortedMap(
        ...
        val groupedStateDeclarations = stateDeclarations.groupBy { it.superTypes.single().toString() }
            .filter { (superType, _) -> superType in familyNames }
        return groupedStateDeclarations.map { (stateName, declarationsOfThisFamily) ->
            ...
            val sortedDeclarations =
                sortStateDeclarations(declarationsOfThisFamily, predecessorMap) // sort declarations
