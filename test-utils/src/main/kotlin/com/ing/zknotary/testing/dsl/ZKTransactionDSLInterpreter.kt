 ...
 */
@DoNotImplement
public interface ZKTransactionDSLInterpreter : ZKVerifies, ZKOutputStateLookup {
    /**
 ...
 * */
public class ZKTransactionDSL<out T : ZKTransactionDSLInterpreter>(interpreter: T, private val notary: Party) :
     ...
     */
    public fun reference(stateLabel: String): Unit = reference(retrieveOutputStateAndRef(ZKContractState::class.java, stateLabel).ref)
     ...
     */
    public fun reference(contractClassName: ContractClassName, state: ZKContractState) {
        val transaction = ledgerInterpreter._unverifiedTransaction(null, ZKTransactionBuilder(notary)) {
     ...
     * @param stateLabel The label of the output state specified when calling [ZKTransactionDSLInterpreter.output] and friends.
     */
    public fun input(stateLabel: String): Unit = input(retrieveOutputStateAndRef(ZKContractState::class.java, stateLabel).ref)

    public fun input(contractClassName: ContractClassName, stateLabel: String) {
        val stateAndRef = retrieveOutputStateAndRef(ZKContractState::class.java, stateLabel)
     ...
     * @param state The state to be added.
     */
    public fun input(contractClassName: ContractClassName, state: ZKContractState) {
        val transaction = ledgerInterpreter._unverifiedTransaction(null, ZKTransactionBuilder(notary)) {
            output(contractClassName, null, notary, null, AlwaysAcceptAttachmentConstraint, state)
        }
     ...
     */
    public fun output(contractClassName: ContractClassName, label: String, notary: Party, contractState: ZKContractState): Unit =
        output(contractClassName, label, notary, null, AutomaticPlaceholderConstraint, contractState)

    /**
     * Adds a labelled output to the transaction.
     */
    public fun output(contractClassName: ContractClassName, label: String, encumbrance: Int, contractState: ZKContractState): Unit =
        output(contractClassName, label, notary, encumbrance, AutomaticPlaceholderConstraint, contractState)

    /**
     * Adds a labelled output to the transaction.
     */
    public fun output(contractClassName: ContractClassName, label: String, contractState: ZKContractState): Unit =
        output(contractClassName, label, notary, null, AutomaticPlaceholderConstraint, contractState)

    /**
     * Adds an output to the transaction.
     */
    public fun output(contractClassName: ContractClassName, notary: Party, contractState: ZKContractState): Unit =
        output(contractClassName, null, notary, null, AutomaticPlaceholderConstraint, contractState)

    /**
     * Adds an output to the transaction.
     */
    public fun output(contractClassName: ContractClassName, encumbrance: Int, contractState: ZKContractState): Unit =
        output(contractClassName, null, notary, encumbrance, AutomaticPlaceholderConstraint, contractState)

    /**
     * Adds an output to the transaction.
     */
    public fun output(contractClassName: ContractClassName, contractState: ZKContractState): Unit =
        output(contractClassName, null, notary, null, AutomaticPlaceholderConstraint, contractState)
