...
class TargetHolderCancelTransferFlowTests : FlowTest() {
    ...
    override fun initialize() {
        ...
        Pipeline
            ...
            .run(bankNodeB) {
                val requestedTransfer = TRANSFER_REQUEST
                RequestTransferFlow.Initiator(requestedTransfer)
            }
            .run(bankNodeB) {
                val requestedTransfer = it.singleOutRefOfType<Transfer>()
                val cancelledTransfer = requestedTransfer.state.data.cancelTransfer()
                CancelTransferFlow.Initiator(requestedTransfer, cancelledTransfer)
            }
            .finally {
                transaction = it
    ...
    @Test
    fun `CancelTransferFlow should be signed by the transferee`() {
        transaction.verifyRequiredSignatures()
    }

    @Test
    fun `CancelTransferFlow should record a transaction for the transferer and the transferee`() {
    ...
    @Test
    fun `CancelTransferFlow should record a transfer state for the transferer and the transferee`() {
        listOf(bankNodeA, bankNodeB).forEach {
            it.transaction {
                val recordedTransaction = it.services.validatedTransactions.getTransaction(transaction.id)
                    ?: fail("Failed to find a recorded transaction with ID: ${transaction.id}.")
                ...
                assertEquals(recordedTransfer.status, TransferStatus.CANCELLED, "Transfer statuses are not equal.")
            }
        }
    }
}
