...
class CancelDepositFlowTests : FlowTest() {
    ...
    override fun initialize() {
        ...
        Pipeline
            ...
            .run(bankNodeA) {
                val paymentRejectedDeposit = it.singleOutRefOfType<Deposit>()
                val cancelledDeposit = paymentRejectedDeposit.state.data.cancelDeposit()
                CancelDepositFlow.Initiator(paymentRejectedDeposit, cancelledDeposit)
            }
            .finally {
                transaction = it
    ...
    @Test
    fun `AdvanceDepositFlow should be signed by the depositor`() {
    ...
    fun `AdvanceDepositFlow should record a deposit state for the depositor, token issuing entity and the custodian`() {
        listOf(bankNodeA, tieNode, custodianNode).forEach {
            it.transaction {
                ...
                assertEquals(recordedDeposit.status, DepositStatus.DEPOSIT_CANCELLED, "Deposit statuses are not equal.")
            }
        }
    }
}
