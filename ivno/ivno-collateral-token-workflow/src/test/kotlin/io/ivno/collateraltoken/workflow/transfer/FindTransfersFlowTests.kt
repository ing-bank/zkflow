...
class FindTransfersFlowTests : FlowTest() {

    private val linearId1 = UniqueIdentifier("Transfer001")
    private val linearId2 = UniqueIdentifier("Transfer001")
    ...
    @Test
    fun `FindTransfersFlow should return the correct number of transfer states when specifying the linearId`() {
        Pipeline
            .create(network)
            .run(bankNodeA) { FindTransfersFlow(linearId = linearId1) }
            .finally { assertEquals(1, it.size) }
    }

    @Test
    fun `FindTransfersFlow should return the correct number of transfer states when specifying the externalId`() {
        Pipeline
            .create(network)
