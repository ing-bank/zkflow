...
class RequestRedemptionFlowTests : FlowTest() {
    ...
    override fun initialize() {
        ...
        Pipeline
            ...
            .finally {
                transaction = it
                redemption = it.singleOutRefOfType()
            }
    }

    @Test
    fun `RequestRedemptionFlow should be signed by the custodian`() {
        transaction.verifySignaturesExcept(REDEMPTION.tokenIssuingEntity.owningKey, REDEMPTION.redeemer.owningKey)
    }

    @Test
    fun `RequestRedemptionFlow should be signed by the token issuing entity`() {
        transaction.verifySignaturesExcept(REDEMPTION.custodian.owningKey, REDEMPTION.redeemer.owningKey)
    }

    @Test
    fun `RequestRedemptionFlow should be signed by the redeemer`() {
        transaction.verifySignaturesExcept(REDEMPTION.tokenIssuingEntity.owningKey, REDEMPTION.custodian.owningKey)
    }

    @Test
    fun `RequestRedemptionFlow should record a transaction for the redeemer, custodian and token issuing entity`() {
    ...
    @Test
    fun `RequestRedemptionFlow should record a transfer state for the redeemer, custodian and token issuing entity`() {
        listOf(bankNodeB, custodianNode, tieNode).forEach {
            it.transaction {
                val recordedTransaction = it.services.validatedTransactions.getTransaction(transaction.id)
                    ?: fail("Failed to find a recorded transaction with ID: ${transaction.id}.")
                ...
                assertEquals(recordedRedemption.status, RedemptionStatus.REQUESTED, "Redemption status is not equal.")
            }
        }
    }
}
