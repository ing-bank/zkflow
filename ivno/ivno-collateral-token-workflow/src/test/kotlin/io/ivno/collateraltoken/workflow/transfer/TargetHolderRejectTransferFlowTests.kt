...
class TargetHolderRejectTransferFlowTests : FlowTest() {
    ...
    @Test
    fun `RejectTransferFlow should be signed by the transferee`() {
        transaction.verifyRequiredSignatures()
    }

    @Test
    fun `RejectTransferFlow should record a transaction for the transferer and the transferee`() {
    ...
    @Test
    fun `RejectTransferFlow should record a transfer state for the transferer and the transferee`() {
        listOf(bankNodeA, bankNodeB).forEach {
            it.transaction {
                val recordedTransaction = it.services.validatedTransactions.getTransaction(transaction.id)
                    ?: fail("Failed to find a recorded transaction with ID: ${transaction.id}.")
