...
class IssueDepositPaymentFlow(
    ...
    class Initiator(
        ...
        private companion object {
            object ISSUING : ProgressTracker.Step("Issuing deposit payment.") {
                override fun childProgressTracker(): ProgressTracker = tracker()
            }
        }

        override val progressTracker: ProgressTracker = ProgressTracker(ISSUING)

        @Suspendable
        override fun call(): SignedTransaction {
            currentStep(ISSUING)
            return subFlow(
                IssueDepositPaymentFlow(
                    oldDeposit,
                    newDeposit,
