...
class UpdateIvnoTokenTypeFlowHandler(
    ...
    @InitiatedBy(UpdateIvnoTokenTypeFlow.Initiator::class)
    private class Handler(private val session: FlowSession) : FlowLogic<SignedTransaction>() {

        private companion object {
            object HANDLING : ProgressTracker.Step("Handling token type update.") {
        ...
        @Suspendable
        override fun call(): SignedTransaction {
            return subFlow(UpdateIvnoTokenTypeFlowHandler(session, HANDLING.childProgressTracker()))
        }
    }
}
